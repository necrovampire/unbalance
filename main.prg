compiler_options _extended_conditions, _use_cstyle_matrix;
...
BEGIN
  ...
  DBHandler = db_open(dbPath, dbms_sqlite, db_readonly);
  if (!DBHandler)
    exit("Error al conectar a la base de datos",0);
  end
  return(DBHandler);
END
...
BEGIN
  ...
  return( consultarResolcionEnDB(query) );
END
...
BEGIN
    ...
    if(!mode_exists(screen.width, screen.height, screen.bpp) )
        exit("modo no soportado",0);
    end
    mode_set(screen.width, screen.height, screen.bpp, screen.flagsmode);
END
